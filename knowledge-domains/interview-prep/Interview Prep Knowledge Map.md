---
title: Interview Prep Knowledge Map
tags: ["interview", "java", "ai", "knowledge-map"]
difficulty: intermediate
estimated_time: 10 min
last_reviewed: 2026-04-20
---

# Interview Prep Knowledge Map

这个目录统一收纳技术面试相关内容。原则是：Java 后端、AI 应用开发、后端工程、系统实战类面试内容优先放在这里；财务、审计、内审这类不同领域的岗位面试单独放在 [[Finance Career Interview Knowledge Map]]。

## Technical Interview

- [[Java Interview Questions]]
- [[AI Application Development Interview Questions]]

## Java Backend Interview

- [[Java Core Interview Answers]]
- [[Java Collections Interview Answers]]
- [[Java Concurrency Interview Answers]]
- [[Java JVM Interview Answers]]
- [[Java Spring Interview Answers]]
- [[Java IO and Network Interview Answers]]
- [[Java Database and Persistence Interview Answers]]
- [[Java Redis Interview Answers]]
- [[Java Security Interview Answers]]
- [[Java Distributed Systems Interview Answers]]
- [[Java Production Engineering Interview Answers]]

## AI Application Development Interview

AI 应用开发面试重点不是“会不会调模型 API”，而是能不能把 LLM、RAG、Agent、工具协议、安全、评估和上线治理讲成一个完整系统。

先看 [[AI Application Development Interview Questions]]，再按需要回到 GenAI 目录补基础：

- [[LLM]]
- [[Prompt Engineering]]
- [[RAG]]
- [[Vector Database]]
- [[AI Agent]]
- [[Tool Use]]
- [[GenAI System Architecture]]

## How to Add Future Interview Content

如果以后新增新的面试主题，优先按下面方式放：

- 语言/后端面试：放在 Technical Interview 或单独建语言模块。
- AI/LLM/Agent 面试：放在 AI Application Development Interview。
- 系统设计实战题：如果是通用系统设计方法，仍放 System Design；如果是“面试参考答案/题库”，可以在这里加入口。
- 职能/行业岗位面试：如果是财务、审计、内审或内控，放到 [[Finance Career Interview Knowledge Map]]；其他领域可以新增单独 tab。
- Behavioral / stakeholder / HM screen：可以在这里加入口，同时保留 Career Development 的沟通成长内容。

## Suggested Paths

- Java 后端面试：从 [[Java Interview Questions]] 开始，再按 Core、Collections、Concurrency、JVM、Spring、Database、Redis、Distributed、Production 顺序复习。
- AI 应用开发面试：从 [[AI Application Development Interview Questions]] 开始，重点准备 RAG、Agent、MCP/tool、evaluation、安全和上线治理。
## Related

- [[Finance Career Interview Knowledge Map]]
