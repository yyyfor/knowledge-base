...
- Climbing Stairs: 定义 `dp[i]` 为到第 `i` 阶的方法数，转移是 `dp[i] = dp[i - 1] + dp[i - 2]`。这是最基础的一维 DP 建模。
- House Robber: 定义 `dp[i]` 为考虑前 `i` 个房子时的最大收益，转移为“抢当前 + `dp[i - 2]`”和“不抢当前 + `dp[i - 1]`”二选一。
- Longest Increasing Subsequence: 基础解法定义 `dp[i]` 为以 `i` 结尾的 LIS 长度，枚举前面更小元素做转移；若要优化，可引出 `tails` + binary search 的 `O(n log n)` 解法。
