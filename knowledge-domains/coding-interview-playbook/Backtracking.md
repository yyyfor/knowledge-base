...
中等题示例：Combination Sum。题目是给你一组互不相同的数字和目标值 `target`，找出所有和为 `target` 的组合，同一个数字可以重复使用。关键是用路径、剩余值和起始下标定义搜索状态，并在超出目标时及时剪枝。
...
- Subsets: 每个元素只有“选”或“不选”两种决策，可用回溯按索引递进生成整棵选择树。时间复杂度 `O(n * 2^n)`，空间复杂度 `O(n)`，不含结果存储。
- Combination Sum: 路径里维护当前和，递归时可重复选择当前数字；一旦和超过目标值就剪枝。时间复杂度与解空间规模相关，最坏可视为指数级，空间复杂度 `O(target)` 到 `O(path length)`。
- N-Queens: 每一行尝试放一个皇后，并用列、主对角线、副对角线集合判断是否合法，不合法立即回退。时间复杂度最坏近似 `O(n!)`，空间复杂度 `O(n)`，不含结果存储。
