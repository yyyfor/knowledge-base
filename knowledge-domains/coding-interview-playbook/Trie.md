...
- Implement Trie (Prefix Tree): 直接实现 `insert`、`search` 和 `startsWith` 三个原语，展示 Trie 的基本节点结构和前缀路径搜索。
- Word Search II: 先把词典建成 Trie，再从棋盘每个起点 DFS。搜索过程中一旦 Trie 中不存在后续字符，就立刻剪枝。
- Search Suggestions System: 把商品词典放进 Trie，输入每个前缀时沿路径向下，并从对应子树取字典序最小的若干候选词。
