...
中等题示例：Group Anagrams。题目是给你一组字符串，把所有字母异位词分到同一组返回。核心不是两两比较，而是为每个单词构造一个稳定 key，例如排序后的字符串或 26 维频率签名。
...
- Valid Anagram: 用频率表统计两个字符串各字符出现次数，最后比较计数是否一致。若字符集固定，也可用定长数组代替 hashmap。时间复杂度 `O(n)`，空间复杂度 `O(1)` 或 `O(charset)`。
- Group Anagrams: 用排序后的字符串或 26 维频率向量作为 key，把同组单词聚到同一个 hashmap bucket 中。若 key 用排序串，时间复杂度约为 `O(n * k log k)`，空间复杂度 `O(nk)`，其中 `k` 是单词平均长度。
